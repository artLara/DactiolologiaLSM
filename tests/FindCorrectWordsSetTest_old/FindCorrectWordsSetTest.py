...
class FindCorrectWordsSetTest():
	...
	def runTest(self, startLoop, endLoop, path2save='noiseTextWordsResult/', path='noiseWordsPkl/'):
		...
		for randomLettersRate in np.arange(startLoop, endLoop, 0.1): #Iterate every letter rates
			randomLettersRate = round(randomLettersRate, 1)
			for maxRandoms in range(2, 8): #Iterate every letter randoms
				...
				with open(path+'noiseWords{}_{}.pkl'.format(randomLettersRate, maxRandoms), 'rb') as inp:
					noiseWords = pickle.load(inp)
				print('Testing {}_{}'.format(randomLettersRate, maxRandoms))
